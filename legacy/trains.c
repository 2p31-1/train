...
void sync_tr(train* a){
	if(a->speed>=0){
		digitalWrite(a->fPin,1);
		digitalWrite(a->pPin,0);
	}else{
		digitalWrite(a->fPin,1);
		digitalWrite(a->pPin,0);	
	}
	softPwmWrite(a->pwm,(int)(fabs(a->speed)/3)+15);
	if(a->fPin>0)printf("%d %d %d %d\n",a->fPin,a->pPin,a->pwm,(int)(fabs(a->speed)/1.5)+15);
	return;
}
void ato(train* a){
